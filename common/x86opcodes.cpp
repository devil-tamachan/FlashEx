...
struct x86opcode
{
  ...
  char jump; //determins if it is a call or jump instruction, 0:no, 1:call(or other functions to be ignored), 2:conditional jump, 3:unconditional jump
...
int FollowInstruction(unsigned char *instructions, unsigned long *eip)
{
  ...
  while(1)
  {
    ...
    {
      if(instructions[*eip] == 0x66) operandOverridePrefix = true;
      if(instructions[*eip] == 0x67) addressOverridePrefix = true;
      (*eip)++;
      continue;
    }
    break;
  ...
  if (opcodes[opcode].type == 1)
  {
    ...
    if (((modregrm&0x07) == 0x04)&&((modregrm&0xC0) != 0xC0))
    {
      ...
      if (((modregrm&0xC0) == 0x00)&&((sib&07)==0x05))
        displacement = 4;
    }
...
int SimulateStackInstruction(
  unsigned char *instructions,
  unsigned long *eip,
  unsigned long *esp,
  unsigned long *retparam)
{
  unsigned char modregrm, sib;
  unsigned short opcode = 0;
  bool operandOverridePrefix = false;
  ...
  // can't handle such instructions currently
  if (addressOverridePrefix&&((opcode == 0x00)||(opcode == 0x20)))
    return 0;

  // don't handle jump/call functions (and similar)
  if (opcodes[opcode].jump)
    return 0;
  ...
  if (opcode==0xFF)
  {
    modregrm = instructions[*eip];
    // indirect CALL
    if ((modregrm&0x38) == 0x10)
      return 0;
    if ((modregrm&0x38) == 0x18)
      return 0; //indirect CALL
    if ((modregrm&0x38) == 0x20)
      return 0; //indirect JUMP
    if ((modregrm&0x38) == 0x28)
      return 0; //indirect JUMP
  }
  ...
  {
    // hack, fix for certain types of mov instruction
    argsize += addressOverridePrefix ? 2 : 4;
  }
  else if (operandOverridePrefix && (opcodes[opcode].argSize == 4))
    argsize += 2;
  else if (operandOverridePrefix && (opcodes[opcode].argSize == 6))
  ...
  (*esp) += opcodes[opcode].stackChange * 4;
  if (opcode == 0xFF)
  {
    if ((modregrm&0x38) == 0x30)
      (*esp) -= 4;
  }

  // handle add esp,const and sub esp,const instructions
  if (opcode==0x81)
  {
    size_t delta = operandOverridePrefix ? 2 : 4;
    unsigned short stackoffset = *((unsigned short *)(&instructions[(*eip)-delta]));
    if (modregrm == 0xC4)
      (*esp) += stackoffset;
    else if (modregrm==0xEC)
      (*esp) -= stackoffset;
  }

  if (opcode==0x83)
  {
    unsigned char stackoffset = ((unsigned char *)instructions)[(*eip)-1];
    if (modregrm==0xC4)
      (*esp) += stackoffset;
    else if (modregrm==0xEC)
      (*esp) -= stackoffset;
  ...
  // check if we reached the return instruction
  if ((opcode == 0xC2) || (opcode == 0xCA))
  {
    *retparam = *((unsigned short *)(&instructions[(*eip)-2]));
    return 2;
  }
...
int CheckCallArguments(
  ...
  unsigned long key)
{
  unsigned char *eip = (unsigned char *)callAddress;
  unsigned long targetAddr = 0;
  unsigned char modregrm, sib = 0;
  unsigned long displacement = 0;
  bool memread = true;

  if((*eip)!=0xFF) return 0;
  eip++;

  modregrm = (*eip);
  eip++;
  if((modregrm & 0x38) != 0x10) return 0;

  switch(modregrm & 0xC7)
  {
  case 0x00:
    targetAddr = registers[7];
    break;
  case 0x40:
    targetAddr = registers[7];
    displacement = 1;
  ...
  if (sib)
  {
    ...
    //add n*index
    targetAddr += registers[7-((sib&0x38)>>3)]*n;
    ...
    case 0x00:
      if((sib & 0x07)==0x05)
      {
        ...
        targetAddr += registers[7-(sib & 0x07)];
      }
      break;
    case 0x40:
      targetAddr += registers[7-(sib & 0x07)] & 0x000000FF;
      break;
    case 0x80:
      targetAddr += registers[7-(sib & 0x07)];
      break;
  ...
  {
    targetAddr += *((unsigned long *)eip);
    eip += 4;
  }

  if(((unsigned long)eip)!=returnAddress)
    return 0;

  if (memread)
  {
    targetAddr = *((unsigned long *)targetAddr);
  }

  if(targetAddr == (functionAddress^key))
  {
    //MessageBoxA(NULL, "Call target check success", "ROPGuard", MB_OK);
    return 1;
  }
  return 0;
}
